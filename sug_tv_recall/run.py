...
if torch.cuda.is_available():
    ...
    uid_dic_path = '/home/admin/workspace/project/odps/bin/uid_dic'
    sug_dic_path = '/home/admin/workspace/project/odps/bin/sug_query_dic'
    summary_path = '/home/admin/workspace/project/dsw_pytorch/sug_tv_recall/log'
    ...
    uid_dic_path = '/Users/tanzhen/Desktop/code/odps/bin/uid_dic'
    sug_dic_path = '/Users/tanzhen/Desktop/code/odps/bin/sug_query_dic'
    summary_path = '/Users/tanzhen/Desktop/code/odps/bin/sug_tv_recall/log'
...
def load_traindata():
    ...
    # ----------------------- uid 列 ---------------------
    t_load_start = time.time()
    df_uid = pd.read_csv(uid_dic_path, sep='\t', names=['uid', 'id'])
    vocab_uid = dict(zip(list(df_uid.uid), list(df_uid.id.astype(np.int64))))
    print("\nFinished Load uid dic size:{}".format(len(vocab_uid)))

    X_uid = item2id(df, col_name='uid', padding_size=1, vocab=vocab_uid)
    t_load_end = time.time()
    print('Uid Data Cost: %s Seconds' % (t_load_end - t_load_start))
    print(X_uid.dtype)
    print("X_uid shape:" + str(X_uid.shape))
    ...
    t_load_end = time.time()
    print('\nSeq Data Cost: %s Seconds' % (t_load_end - t_load_start))
    print(X_seq.dtype)

    target = "label"
    target = df[target].values

    return X_wide, X_deep, prepare_deep, X_seq, X_prefix, X_sug, X_uid, target
...
if __name__ == '__main__':
    X_wide, X_deep, prepare_deep, X_seq, X_prefix, X_sug, X_uid, target = load_traindata()
    print("----------------------------- Start Load eval Data")
    X_eval_wide, X_eval_deep, prepare_eval_deep, X_eval_seq, X_eval_prefix, X_eval_sug, X_eval_uid, eval_target = load_evaldata()
    ...
    if use_head:
        wide_deep_model = WideDeep(wide=wide, deepdense=deepdense, deeptext=transformer, head_layers=[256,128])
    else:
    ...
    uid_opt = torch.optim.Adam(wide_deep_model.uid_embedding.parameters())
    ...
    uid_sch = torch.optim.lr_scheduler.StepLR(uid_opt, step_size=5)
    ...
    if use_head:
        optimizers = {"wide": wide_opt, "deepdense": deep_opt, 'deeptext': text_opt, "prefix_embedding": prefix_opt, "deephead":deephead_opt, "uid_embedding": uid_opt}
        schedulers = {"wide": wide_sch, "deepdense": deep_sch, 'deeptext': text_sch, "prefix_embedding": prefix_sch, "deephead":deephead_sch, "uid_embedding": uid_sch}
        initializers = {"wide": KaimingNormal, "deepdense": XavierNormal, 'deeptext': KaimingNormal, "prefix_embedding": KaimingNormal, "deephead":KaimingNormal, "uid_embedding": KaimingNormal}
    else:
    ...
    X_train = {"X_wide": X_wide, "X_deep": X_deep, "X_text": X_seq, "X_prefix": X_prefix, "X_uid": X_uid, "X_sug":X_sug, "target": target}
    X_val = {"X_wide": X_eval_wide, "X_deep": X_eval_deep, "X_text": X_eval_seq, "X_prefix": X_eval_prefix, "X_uid": X_eval_uid, "X_sug":X_eval_sug, "target": eval_target}
